...
async function switchTab(tabName, clickedElement = null) {
  ...
  switch(tabName) {
    ...
    case 'clubs':
      ...
      await renderClubs(); // renderClubs already loads fresh data
      break;
    case 'classes':
        ...
        ${inactiveClasses.length > 0 ? `
          ...
          <tr style="border-bottom: 1px solid #e0e0e0; opacity: 0.7; background: #f9f9f9;">
            ...
            <td style="padding: 12px 8px; text-align: left;">
              ...
              <button onclick="removeClass(${cls.ID})" class="btn btn-sm btn-danger" style="margin-left: 5px;">${cls.IsMultiSession ? 'Remove All' : 'Remove'}</button>
            </td>
          </tr>
        `).join('')}
        ` : ''}
...
async function handleCreateClub(e) {
  ...
  try {
    // Create club without EventID (Admin can link clubs to events later via Events > Manage Clubs)
    const response = await fetchWithAuth('/api/clubs', {
      method: 'POST',
    ...
    if (response.ok) {
      showNotification('Club created successfully. You can link it to events via the Events tab > Manage Clubs.', 'success');
